...
namespace cartservice
{
    class Program
    {
        ...
        static object StartServer(string host, int port, ICartStore cartStore)
        {
            ...
            Task serverTask = Task.Run(async () =>
            {
                try
                {
                    ...
                    logger.Debug($"Cart server is listening at {host}:{port}");
                    server.Start();
        ...
        static void Main(string[] args)
        {
            if (args.Length == 0)
            {
                logger.Error("Invalid number of arguments supplied");
                Environment.Exit(-1);
            }
            ...
            switch (args[0])
            {
                case "start":
                    Parser.Default.ParseArguments<ServerOptions>(args).MapResult(
                        ...
                        {
                            logger.Debug($"Started as process with id {System.Diagnostics.Process.GetCurrentProcess().Id}");

                            // Set hostname/ip address
                            string hostname = options.Host;
                            if (string.IsNullOrEmpty(hostname))
                            {
                                logger.Debug($"Reading host address from {CART_SERVICE_ADDRESS} environment variable");
                                hostname = Environment.GetEnvironmentVariable(CART_SERVICE_ADDRESS);
                                if (string.IsNullOrEmpty(hostname))
                                {
                                    logger.Debug($"Environment variable {CART_SERVICE_ADDRESS} was not set. Setting the host to 0.0.0.0");
                                    hostname = "0.0.0.0";
                                }
                            }
                            ...
                            if (options.Port <= 0)
                            {
                                logger.Debug($"Reading cart service port from {CART_SERVICE_PORT} environment variable");
                                string portStr = Environment.GetEnvironmentVariable(CART_SERVICE_PORT);
                                if (string.IsNullOrEmpty(portStr))
                                {
                                    logger.Debug($"{CART_SERVICE_PORT} environment variable was not set. Setting the port to 8080");
                                    port = 8080;
                                }
                                else
                                {
                            ...
                            {
                                logger.Debug("Redis cache host(hostname+port) was not specified. Starting a cart service using local store");
                                logger.Debug("If you wanted to use Redis Cache as a backup store, you should provide its address via command line or REDIS_ADDRESS environment variable.");
                                cartStore = new LocalCartStore();
                            }
        ...
        private static string ReadRedisAddress(string address)
        {
            ...
            logger.Debug($"Reading redis cache address from environment variable {REDIS_ADDRESS}");
