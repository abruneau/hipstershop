...
namespace cartservice.cartstore
{
    public class RedisCartStore : ICartStore
    {
        ...
        private const int REDIS_RETRY_NUM = 4;
        ...
        private void EnsureRedisConnected()
        {
            ...
            lock (locker)
            {
                ...
                logger.Debug("Connecting to Redis: " + connectionString);
                redis = ConnectionMultiplexer.Connect(redisConnectionOptions);

                if (redis == null || !redis.IsConnected)
                {
                    logger.Error("Wasn't able to connect to redis");
                ...
                logger.Debug("Performing small test");
                cache.StringSet("cart", "OK");
                object res = cache.StringGet("cart");
                logger.Debug($"Small test result: {res}");

                redis.InternalError += (o, e) => { logger.Error(e.Exception); };
                redis.ConnectionRestored += (o, e) =>
                {
                    isRedisConnectionOpened = true;
                    logger.Debug("Connection to redis was retored successfully");
                };
                redis.ConnectionFailed += (o, e) =>
                {
                    logger.Error("Connection failed. Disposing the object");
